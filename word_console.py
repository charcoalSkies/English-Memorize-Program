...
def save_wrong_answers(word_dict, day):
    try:
        with open(f"/Users/mac/Dev/Project/RandomWord/word_wrong/eDay{day}.txt", 'w', encoding='utf-8') as f:
            for question in wrong_answers_global:
                f.write(f"{question} | {', '.join(word_dict[question])}\n")
        print(f"틀린 답이 'eDay{day}.txt' 파일에 저장되었습니다.\n")
        subprocess.run(["open", "/Users/mac/Dev/Project/RandomWord/word_wrong"])
...
def play_eng_to_kor(word_dict):
    ...
    questions = list(word_dict.keys())
    random.shuffle(questions)  # 단어 리스트를 랜덤하게 섞음
